...
public class KnifetableRecipe implements Recipe<SimpleContainer> {
    ...
    private KnifetableRecipe(NonNullList<Ingredient> inpuItem, NonNullList<Integer> ingredientCounts, ItemStack output, ResourceLocation id) {
    ...
    public boolean matches(SimpleContainer container, Level level) {
        for (int i = 0; i < INPUT_SLOTS; i++) {
            ...
            if (!ingredient.test(stack) || stack.getCount() < Math.max(1, ingredientCounts.get(i))) {
                return false;
            }
        }
        return true;
    ...
    public RecipeSerializer<?> getSerializer() {
        return RecipreMod.KNIFETABLE_SERIALEZATOR.get();
    }

    @Override
    public RecipeType<?> getType() {
        return RecipreMod.KNIFETABLE_TYPE.get();
    }
    public static class Type implements RecipeType<KnifetableRecipe> {
        public static final ResourceLocation ID = new ResourceLocation(cat_mod.MODID, "knifetable");
        public static final Type INSTANCE = new Type();
    }

    public static class Serializer implements RecipeSerializer<KnifetableRecipe> {
        public static final Serializer INSTATE = new Serializer();
        public static final ResourceLocation ID = new ResourceLocation(cat_mod.MODID,"knifetable");

        @Override
        public KnifetableRecipe fromJson(ResourceLocation resourceLocation, JsonObject jsonObject) {
            ...
            return new KnifetableRecipe(inputs, counts, output, resourceLocation);
        }

        @Override
        public @Nullable KnifetableRecipe fromNetwork(ResourceLocation resourceLocation, FriendlyByteBuf friendlyByteBuf) {
            NonNullList<Ingredient> inputs = NonNullList.withSize(SLOT_COUNT, Ingredient.EMPTY);
            NonNullList<Integer> counts = NonNullList.withSize(SLOT_COUNT, 0);
        ...
        @Override
        public void toNetwork(FriendlyByteBuf friendlyByteBuf, KnifetableRecipe cookingtableRecipe) {
            for (int i = 0; i < SLOT_COUNT; i++) {
