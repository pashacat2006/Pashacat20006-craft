 ...
 */
public class KnifetableMenu extends AbstractContainerMenu {
    public final KnifetableEntity blockenty;
    private final Level l;
    private final ContainerData dat;

    /** Конструктор для клиента — позиция блока приходит из сети */
    public KnifetableMenu(int ContanirId, Inventory inv, FriendlyByteBuf extradat) {
        this(ContanirId, inv, inv.player.level().getBlockEntity(extradat.readBlockPos()), new SimpleContainerData(2));
    }

    /** Конструктор для сервера */
    public KnifetableMenu(int contanirid, Inventory inv, BlockEntity entity, ContainerData dat1) {
        super(MenuType.KNIFETABLE_MENU.get(), contanirid);
        checkContainerSize(inv, 3);
        blockenty = ((KnifetableEntity) entity);
        ...
        this.addSlot(new SlotItemHandler(handler, 0, 0, 12 - 37));  // Butter / масло
        this.addSlot(new SlotItemHandler(handler, 1, 195, 12 - 37));  // Food / еда
        this.addSlot(new SlotItemHandler(handler, 2, 90, 165)); // Результат
    ...
    private void addPlayerInventory(Inventory playerInventory) {
        for (int i = 0; i < 3; ++i) {
            for (int l = 0; l < 9; ++l) {
                this.addSlot(new Slot(playerInventory, l + i * 9 + 9, 12 + l * 18, 100 + i * 18));
            }
        }
    }
    ...
    private void addPlayerHotbar(Inventory playerInventory) {
        for (int i = 0; i < 9; ++i) {
            this.addSlot(new Slot(playerInventory, i, 9 + i * 18, 201));
        }
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(ContainerLevelAccess.create(l, blockenty.getBlockPos()), player, BlockMod.KNIFETABLE_BLOCK.get());
    }
}
